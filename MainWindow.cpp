...
QImage* MainWindow::GetScaledImage(QString imagePath,int scaledWidth /*=  300*/,int scaledHeight/* = 200*/)
{
...
void MainWindow::GetLocalData(QVector<ST_COURSE_ITEM> &vecCourseItem)
{
    for(int nIndex = 2;nIndex < 10;nIndex++)
    {
        ST_COURSE_ITEM item;
        QString thumUrl;
        thumUrl = QString(":/image/images/%1.jpg").arg(nIndex+1);  //QSting  格式化数据使用arg
        qDebug() << thumUrl;
...
void MainWindow::OnGetCourseListResult(const QVector<ST_COURSE_ITEM> &vecCourseItem)
{
    //加在服务端请求过来的数据
    for (int nIndex= 0; nIndex < vecCourseItem.size(); ++nIndex)
    {
...
//     manager.get(request);
 ...
 {
  QMessageBox::information(this,tr("MainWindow"), tr("onCancelClicked!"));
 }
